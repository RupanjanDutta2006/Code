...
export const MobileLandscapeFullscreen: React.FC<MobileLandscapeFullscreenProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 w-screen h-screen h-[100dvh] bg-slate-950 text-slate-100 flex flex-col overflow-hidden select-none pt-[env(safe-area-inset-top)] pb-[env(safe-area-inset-bottom)] pl-[env(safe-area-inset-left)] pr-[env(safe-area-inset-right)]">
      ...
      <div className="flex-1 grid grid-cols-12 gap-2 p-2 min-h-0 overflow-hidden">
        ...
        <div className="col-span-5 flex flex-col gap-2 min-h-0 overflow-hidden">
          ...
          <div className="flex items-center justify-between px-2.5 py-1.5 rounded-xl bg-slate-900/90 border border-slate-800 text-[11px] font-mono">
          ...
          <div className="flex-1 rounded-2xl bg-slate-950 border border-slate-800 flex flex-col min-h-0 overflow-hidden shadow-inner">
            <div
                ...
                return (
                  <div
                    ...
                    className={`flex items-center gap-2 px-1.5 py-0.5 rounded transition-colors ${
                      isActive
                        ? 'bg-brand-500/30 border-l-2 border-brand-400 text-white font-semibold'
          ...
          <div className="h-[95px] rounded-2xl bg-slate-900/95 border border-slate-800 p-2 flex flex-col min-h-0 shadow-inner">
        ...
        <div className="col-span-7 flex flex-col gap-2 min-h-0 overflow-hidden relative">
          ...
          <div className="flex items-center justify-between px-2.5 py-1.5 rounded-xl bg-slate-900/90 border border-slate-800 text-[11px] font-mono">
            ...
            {/* Step Action Tag */}
            <span className="text-[10px] text-brand-300 font-bold truncate max-w-[180px]">
              {currentStep.action}
            </span>
          </div>

          {/* Simulation Visualizer Canvas */}
          <div className="flex-1 rounded-2xl bg-slate-900/60 border border-slate-800/80 p-2 sm:p-3 flex items-center justify-center min-h-0 overflow-hidden relative shadow-2xl">
            ...
            {isCompleted && (
              <div className="absolute inset-0 bg-slate-950/85 backdrop-blur-sm flex flex-col items-center justify-center p-4 text-center space-y-3 z-30 animate-in fade-in duration-200">
                <div className="p-2.5 rounded-2xl bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 animate-bounce">
                ...
                <div className="flex items-center gap-2 pt-1">
                  ...
                  <button
                    ...
                    className="px-3.5 py-1.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold flex items-center gap-1 shadow-md shadow-emerald-500/20"
                  >
            ...
            {activePopover === 'explanation' && (
              <div className="absolute top-2 left-2 right-2 p-3 rounded-2xl bg-slate-900/95 border border-brand-500/40 text-slate-200 text-xs shadow-2xl backdrop-blur-md z-20 space-y-1.5 animate-in fade-in slide-in-from-top-1">
              ...
              <div className="absolute top-2 left-2 right-2 p-3 rounded-2xl bg-slate-900/95 border border-brand-500/40 text-slate-200 text-xs shadow-2xl backdrop-blur-md z-20 space-y-1.5 animate-in fade-in slide-in-from-top-1">
                ...
                <div className="grid grid-cols-3 sm:grid-cols-4 gap-2 pt-1">
                  {Object.entries(currentStep.variables || {}).map(([k, v]) => (
                    <div
                      key={k}
                      className="p-1.5 rounded-xl bg-slate-800/90 border border-slate-700 flex flex-col items-center text-[10px] font-mono"
                    >
      ...
      <div className="h-12 px-3 bg-slate-900/95 border-t border-slate-800 flex items-center justify-between gap-2 shadow-2xl select-none">
        ...
        <div className="flex items-center gap-1">
          ...
          <button
            ...
            className="w-9 h-9 rounded-xl bg-slate-800 hover:bg-slate-700 disabled:opacity-30 text-brand-400 flex items-center justify-center transition-colors"
            title="Previous Step"
          >
          ...
          {isPlaying ? (
            ...
            <button
              onClick={onPlay}
              className="px-3.5 h-9 rounded-xl bg-gradient-to-r from-brand-600 to-indigo-600 hover:from-brand-500 text-white font-bold text-xs flex items-center gap-1.5 shadow-md shadow-brand-500/20"
              title="Play (Space)"
            >
        ...
        {/* Center: Speed Pills */}
        <div className="flex items-center gap-1 bg-slate-800/90 p-0.5 rounded-xl border border-slate-700/80 font-mono text-[10px]">
        ...
        <div className="flex items-center gap-2">
          <span className="px-2 py-1 rounded-lg bg-slate-800 text-[10px] font-mono font-bold text-brand-300">
            {currentStepIndex + 1} / {steps.length}
          </span>

          <button
            onClick={onExitFullscreen}
            className="h-9 px-2.5 rounded-xl bg-slate-800 hover:bg-rose-900/50 hover:text-rose-300 text-slate-300 text-xs font-semibold flex items-center gap-1 transition-colors"
